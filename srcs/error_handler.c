...
void	print_input_err(int err)
{
	...
	if (err == 5)
		printf(" %s- 🛌  [time_to_sleep]%s should be a valid positive number",
			BLD_WHITE, RESET);
	if (err == 6)
		printf(" %s- 🍽️  [min_meals_to...]%s should be a valid positive number",
			BLD_WHITE, RESET);
	if (err > 1 && err < 7)
...
void	print_settings_range_err(int err)
{
	printf("\n");
	if (err == 21)
		printf("- An input value %scannot be 0%s for the simulation to work\n",
			BLD_RED, RESET);
	if (err == 22)
		printf("- You should not simulate with more than %s200%s philos\n",
			BLD_RED, RESET);
	if (err == 23)
